...
describe("[Challenge] The rewarder", function () {
  ...
  let liquidityToken, flashLoanPool, rewarderPool, rewardToken, accountingToken, hack;
  ...
  before(async function () {
    ...
    hack = await (
      await ethers.getContractFactory("HackRewarder")
    ).deploy(flashLoanPool.address, liquidityToken.address, rewarderPool.address);
  });
  ...
  it("Execution", async function () {
    ...
    // snapshot 기록 (withdraw에는 snapshot이 없음)
    await hack.hack();
    // 리워드 받기
    await hack.connect(player).getReward();
  });
